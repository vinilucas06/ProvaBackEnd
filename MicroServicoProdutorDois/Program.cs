...
namespace MicroServicoProdutorDois
{
    class Program
    {
        ...
        {
            Console.WriteLine("==Serviço Produtor 02==");
            Log.Logger = new LoggerConfiguration()
               .WriteTo.File("logsProdutor2/log.txt")
               .CreateLogger();
         ...
         { "bootstrap.servers", "localhost:9092" },
            ...
            using (var producer = new Producer<string, string>(config, new StringSerializer(Encoding.UTF8), new StringSerializer(Encoding.UTF8)))
            {
                ...
                while (text != "exit")
                {
                    ...
                    text = "Hello World";
                    obj = servicoObjetoGenerico.Cadastrar("Produtor - 02");
                    producer.ProduceAsync("hello-topic2", JsonConvert.SerializeObject(obj), text).GetAwaiter().GetResult();
